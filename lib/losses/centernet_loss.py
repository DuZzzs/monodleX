...
def compute_centernet3d_loss(input, target):
    ...
    # for dist
    dist_dict = {}
    dist_dict['seg'] = seg_loss
    dist_dict['offset2d'] = offset2d_loss
    dist_dict['size2d'] = size2d_loss
    dist_dict['offset3d'] = offset3d_loss
    dist_dict['depth'] = depth_loss
    dist_dict['size3d'] = size3d_loss
    dist_dict['heading'] = heading_loss

    total_loss = seg_loss + offset2d_loss + size2d_loss + offset3d_loss + \
                 depth_loss + size3d_loss + heading_loss
    return total_loss, stats_dict, dist_dict
...
def compute_size2d_loss(input, target):
    ...
    if target['mask_2d'].sum() > 0:
        ...
        size2d_loss = F.mse_loss(input['size_2d'], input['size_2d'], reduction='mean')
    return size2d_loss
...
def compute_offset2d_loss(input, target):
    ...
    if target['mask_2d'].sum() > 0:
        ...
        offset2d_loss = F.mse_loss(input['offset_2d'], input['offset_2d'], reduction='mean')
...
def compute_depth_loss(input, target):
    ...
    if target['mask_3d'].sum() > 0:
        ...
        depth_loss = F.mse_loss(input['depth'], input['depth'], reduction='mean')
    return depth_loss
...
def compute_offset3d_loss(input, target):
    ...
    if target['mask_3d'].sum() > 0:
        ...
        offset3d_loss = F.mse_loss(input['offset_3d'], input['offset_3d'], reduction='mean')
...
def compute_size3d_loss(input, target):
    ...
    if target['mask_3d'].sum() > 0:
        ...
        size3d_loss = F.mse_loss(input['size_3d'], input['size_3d'], reduction='mean')
    return size3d_loss
...
def compute_heading_loss(input, target):
    ...
    if mask.sum() > 0:
        ...
        # cls_loss = 0.0
        cls_loss = F.mse_loss(input['heading'], input['heading'], reduction='mean')
        reg_loss = F.mse_loss(input['heading'], input['heading'], reduction='mean')
